...
# a one line clone-and-compile for turtlemarinecoin:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/turtlemarine/turtlemarinecoin/master/multi_installer.sh" | bash
#
...
_fail() {
    ...
    _colorize red "Please check build.log and if you need help so do I'" && echo
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/turtlemarinecoin ]; then
            read -r -p "${1:-turtlemarinecoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old turtlemarinecoin directory" && echo
                    rm -rf "$PWD"/turtlemarinecoin
                    ;;
                *)
                    _fail "turtlemarinecoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir turtlemarinecoin
        git clone -q https://github.com/turtlemarine/turtlemarinecoin turtlemarinecoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd turtlemarinecoin
    fi
}

_build_turtlecoin() {
    _note "Building turtlemarinecoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/turtlemarine')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/turtlemarine')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "turtlemarinecoin Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'turtlemarinecoin/build/src/' for the executible binaries. See 'https://github.com/turtlemarine/turtlemarinecoin' for more project support. Cowabunga!"
